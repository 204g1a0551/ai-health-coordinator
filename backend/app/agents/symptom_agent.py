...
SYMPTOM_PATTERNS = [
    (r"\bheart\s*attack\b|\bcardiac\s+arrest\b|\bmyocardial\s+infarction\b", "Heart Attack / Cardiac Event"),
    (r"\bchest\s+pain\b|\bchest\s+tightness\b|\bchest\s+pressure\b|\bangina\b", "Chest Pain"),
    (r"\bpalpitation(s)?\b|\bracing\s+heart\b|\birregular\s+heartbeat\b", "Heart Palpitations"),
    (r"\bshortness\s+of\s+breath\b|\bdifficulty\s+breathing\b|\bbreathless(ness)?\b|\bwheez(ing)?\b|\basthma\b", "Shortness of Breath"),
    (r"\b(high\s+)?fever(ish)?\b|\bhigh\s+temperature\b|\bpyrexia\b", "Fever"),
    (r"\bmigraine(s)?\b", "Migraine"),
    ...
    (r"\bdizziness\b|\blightheaded(ness)?\b|\bdizzy\b|\bvertigo\b", "Dizziness"),
    ...
    (r"\bacid\s+reflux\b|\bheartburn\b|\bgerd\b|\bgastric\b|\bgastritis\b|\bindigestion\b", "Acid Reflux / Indigestion"),
    (r"\bdiarrhea\b|\bloose\s+stools\b|\bloose\s+motion\b", "Diarrhea"),
    (r"\brash\b|\bskin\s+rash\b|\bitching\b|\bitchy\b|\beczema\b", "Rash"),
